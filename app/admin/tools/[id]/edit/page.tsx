...
export default function EditToolPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchTool() {
      try {
        const response = await fetch(`/api/admin/tools/${params.id}/get`)
        if (!response.ok) throw new Error('Failed to fetch tool')
        const tool = await response.json()

        const screenshots = Array.isArray(tool.screenshots)
          ? tool.screenshots.map((s: any) => (typeof s === 'string' ? s : s.url)).join(', ')
          : ''
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-neutral-600 dark:text-neutral-400">Update tool information</p>
      </div>
